import Unique from "../../../Wolfie2D/DataTypes/Interfaces/Unique";
import Vec2 from "../../../Wolfie2D/DataTypes/Vec2";
import Emitter from "../../../Wolfie2D/Events/Emitter";
import GameNode from "../../../Wolfie2D/Nodes/GameNode";
import Sprite from "../../../Wolfie2D/Nodes/Sprites/Sprite";
import Layer from "../../../Wolfie2D/Scene/Layer";
import Scene from "../../../Wolfie2D/Scene/Scene";
import Puzzle from "./Puzzle";
import AABB from "../../../Wolfie2D/DataTypes/Shapes/AABB";

/*

two ideas:
 a real lockpicking puzzle where you actually rotate a lockpick
OR
you press e which will fill up a bar and the faster you fill up the bar the more it incrases the loudness of the player

*/
export default class Lockpick extends Puzzle {
    boundary: AABB;
    protected completed: boolean;
    protected complettionProgress: number;
    protected completedNeeded: number;
    public constructor(sprite: Sprite, completedNeeded: number) {
        super(sprite);
        this.completed = false;
        this.complettionProgress = 0;
        this.completedNeeded = completedNeeded;
        this.boundary = new AABB();
    }

    updateBoundary(): void {
		this.boundary.center.set(this.position.x, this.position.y);
		this.boundary.halfSize.set(12, 12);
	}

    
    public increaseProgress(): void {
        this.complettionProgress += 1;
        if(this.complettionProgress >= 15){
            this.completed = true;
        }
    }


    public get status(): boolean { return this.completed; }
    public set status(value: boolean) { this.completed = value; }

    public get progress(): number { return this.complettionProgress; }
    public set progress(value: number) { this.complettionProgress = value; }

    public set needed(value: number) { this.completedNeeded = value; }
    public get needed(): number { return this.completedNeeded; }
}