...
export default class Level2 extends HW4Scene {
    ...
    private LockpickHUD: LockpickHUD;
    ...
    private lockpicks: Array<Lockpick>;
    ...
    public override updateScene(deltaT: number): void {
        ...
        for (let locker of this.lockers) {
            if (locker.visible && this.player.collisionShape.overlaps(locker.boundary) && this.player.visible && Input.isKeyJustPressed('e')){
              
                    this.emitter.fireEvent(GameEventType.PLAY_SOUND, {key: "locker", loop: false, holdReference: true});
                    this.player.visible = false;
                
   
            }else if(locker.visible && !this.player.visible && Input.isKeyJustPressed('e') && this.player.collisionShape.overlaps(locker.boundary)){
        ...
        //this.LockpickHUD.update(deltaT);
    ...
    protected initializePlayer(): void {
        ...
        this.player.position.set(40, 150);
        ...
        //Give the player a loudness bar
    ...
    protected enterChase(): void {
        ...
        for (let i = 0; i < this.guards.length; i++) {
            ...
            }else if(this.guards[i].position.distanceTo(this.player.position) < 50 && this.player.loudness > 10 && !distracted){
                this.seenFlag = true;
                this.guards[i].alpha = .99;
                let xpos = this.player.position.x;
                let ypos = this.player.position.y;
    ...
    protected initLockpickActivity(): void {
       //this.lockpick= this.load.getObject("lockpick");
        ...
        this.lockpick.position.set(150, 40);
